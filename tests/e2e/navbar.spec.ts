...
test.describe("Navbar Component", () => {
  test("renders logo and desktop links", async ({ page, isMobile }) => {
    ...
    if (!isMobile) {
      ...
      await expect(desktopNav.getByRole("link", { name: "Book a Discovery Call", exact: false })).toBeVisible();
    } else {
  ...
  test("mobile menu overlay toggle", async ({ page, isMobile }) => {
    ...
    await expect(mobileNav.getByRole("link", { name: "Book a Discovery Call", exact: false })).toBeVisible();
  });
});
